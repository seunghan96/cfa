from data_provider.data_factory_financial import data_provider_financial as data_provider
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(vali_loader):
                ...
                # Text context for models with internal injection (e.g. TSMixer)
                if self.Doc2Vec==False:
                    if self.pool_type=="avg":
                        text_context = prompt_emb.mean(dim=1)
                    elif self.pool_type=="max":
                        text_context = prompt_emb.max(dim=1)[0]
                    elif self.pool_type=="min":
                        text_context = prompt_emb.min(dim=1)[0]
                    else:
                        text_context = prompt_emb.mean(dim=1)
                else:
...
#             test_loss = self.vali(test_data, test_loader, criterion)
    ...
    def test(self, setting, test=0):
        ...
        # result save
        folder_path = './results_financial/' + setting + '/'
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
