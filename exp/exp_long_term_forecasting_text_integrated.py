...
# Import text-integrated models
import sys
import os
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from models import Informer, Transformer, Autoformer, TimesNet, Nonstationary_Transformer, DLinear, FEDformer, \
    Reformer, PatchTST, Crossformer, FiLM, iTransformer, \
...
class Exp_Long_Term_Forecast(Exp_Basic):
    def __init__(self, args):
        ...
        self.hug_token="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"
        
        
        # Text injection configuration
        self.text_injection_mode = getattr(configs, 'text_injection_mode', 'last')
        configs.text_injection_mode = self.text_injection_mode
        ...
        if configs.llm_model == 'Doc2Vec':
            ...
            # Prepare documents for Doc2Vec - preserve existing behavior if already tokenized
            documents = []
            for i, doc in enumerate(result[:train_len]):
                # If doc is already a list of strings (tokenized), use as is
                if isinstance(doc, list) and len(doc) > 0 and isinstance(doc[0], str):
                    documents.append(TaggedDocument(doc, [i]))
                # If doc is a string, tokenize it
                elif isinstance(doc, str):
                    tokens = doc.split() if doc else ['null']
                    documents.append(TaggedDocument(tokens, [i]))
                # Otherwise, convert to string then tokenize
                else:
                    doc_str = str(doc) if doc else 'null'
                    tokens = doc_str.split() if doc_str else ['null']
                    documents.append(TaggedDocument(tokens, [i]))
            ...
            if configs.llm_model == 'LLAMA2':
                ...
                cache_path = "./"
                #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
    ...
    def _build_model(self):
        # Use text-integrated models
        text_integrated_model_dict = {
            'TimesNet': TimesNet,
        ...
        model = text_integrated_model_dict[self.args.model].Model(self.args).float()
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(vali_loader):
                ...
                if self.Doc2Vec==False:
                    ...
                    # Doc2Vec expects tokenized word lists (list of strings)
                    # Handle different input formats while preserving existing behavior
                    prompt = []
                    for text in batch_text:
                        # If already a list of strings (tokenized), use as is
                        if isinstance(text, list) and len(text) > 0 and isinstance(text[0], str):
                            prompt.append(text)
                        # If numpy array or other non-string type, convert to string then tokenize
                        elif isinstance(text, (np.ndarray, np.generic)) or not isinstance(text, str):
                            text_str = str(text.item() if isinstance(text, np.ndarray) and text.size == 1 else text)
                            tokens = text_str.split() if text_str else ['null']
                            prompt.append(tokens)
                        # If string, tokenize it
                        elif isinstance(text, str):
                            tokens = text.split() if text else ['null']
                            prompt.append(tokens)
                        else:
                            # Fallback: convert to string and tokenize
                            tokens = str(text).split() if text else ['null']
                            prompt.append(tokens)
                ...
                # Text context: use mean pooling to get global text embedding [B, text_embedding_dim]
                if self.Doc2Vec==False:
                    # Pool over token dimension to get [B, text_embedding_dim]
                    if self.pool_type=="avg":                
                        text_context = prompt_emb.mean(dim=1)  # [B, text_embedding_dim]
                    elif self.pool_type=="max":
                        text_context = prompt_emb.max(dim=1)[0]  # [B, text_embedding_dim]
                    elif self.pool_type=="min":
                        text_context = prompt_emb.min(dim=1)[0]  # [B, text_embedding_dim]
                    else:  # default to mean
                        text_context = prompt_emb.mean(dim=1)  # [B, text_embedding_dim]
                else:
                    text_context = prompt_emb.squeeze(1) if len(prompt_emb.shape) == 3 else prompt_emb
                
                # decoder input
                dec_inp = torch.zeros_like(batch_y[:, -self.args.pred_len:, :]).float()
                dec_inp = torch.cat([batch_y[:, :self.args.label_len, :], dec_inp], dim=1).float().to(self.device)
                
                # encoder - decoder with text_context
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if self.args.output_attention:
                            outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, text_context=text_context)[0]
                        else:
                            outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, text_context=text_context)
                else:
                    if self.args.output_attention:
                        outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, text_context=text_context)[0]
                    else:
...
#         np.save(folder_path + 'true.npy', trues)
        ...
        if os.path.exists(checkpoint_path):
            try:
                ...
                print(f'Error deleting checkpoint folder {checkpoint_path}: {e}')
