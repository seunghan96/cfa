...
import torch
from exp.exp_long_term_forecasting_financial import Exp_Long_Term_Forecast
...
if __name__ == '__main__':
    ...
    # data loader
    parser.add_argument('--data', type=str, default='custom_financial', help='dataset type')
    parser.add_argument('--root_path', type=str, default='./data/Financial/', help='root path of the data file')
    parser.add_argument('--data_path', type=str, default='crude_oil_WTI.csv', help='data file')
...
parser.add_argument('--num_workers', type=int, default=10, help='data loader num workers')
    ...
    # Text integrated parameters
    parser.add_argument('--use_text_integrated', type=int, default=0, help='use text-integrated models (1) or original models (0)')
    parser.add_argument('--text_injection_mode', type=str, default='last', help='text injection mode: first, middle, last, first-middle, first-last, middle-last, or first-middle-last')
    parser.add_argument('--cfa_reduction', type=int, default=8, help='reduction ratio for CFA adapter bottleneck (default: 8)')

    # Learning rate multiplier
    parser.add_argument('--lr_multiplier', type=float, default=1.0, help='multiplier for all learning rates (default: 1.0, use 0.1 for 10x smaller, 10.0 for 10x larger)')
    ...
    if args.lr_multiplier != 1.0:
        ...
        args.learning_rate2 *= args.lr_multiplier
        args.learning_rate3 *= args.lr_multiplier
        args.learning_rate_weight *= args.lr_multiplier
        print(f"Applied LR multiplier {args.lr_multiplier}x: learning_rate={args.learning_rate}, learning_rate2={args.learning_rate2}, learning_rate3={args.learning_rate3}, learning_rate_weight={args.learning_rate_weight}")
    ...
    if args.is_training:
        for ii in range(args.itr):
            if args.use_text_integrated:
                version_suffix = f"{args.text_injection_mode}"
            else:
                version_suffix = "voriginal"
            # Add LR multiplier to suffix if not 1.0
            lr_suffix = f"_lr{args.lr_multiplier}x" if args.lr_multiplier != 1.0 else ""
            setting = '{}_{}_{}_ft{}_sl{}_ll{}_pl{}_dm{}_nh{}_el{}_dl{}_df{}_expand{}_dc{}_fc{}_eb{}_dt{}_{}_{}_llm{}_{}{}'.format(
                args.model_id,
                args.model,
                args.data,
                args.features,
